...
7. **Features, one at a time.** The first is always sign-in. Each one is checked for design consistency and security (including that one user can't see another's data) before it's done. Then Claude tells you what to click to try it, and asks whether to put it live. It won't put something live with a serious security problem. After it goes live, Claude checks the live app and asks you to click through it once. If something broke, it can undo the release while it finds the cause.
8. **Keep improving.** Everything you'd like next lives in `docs/BACKLOG.md`. Claude recommends what to do next and you can pick something else. Small tweaks ("change this wording", "this page is slow") take a lighter, quicker path than a whole feature. About once a month Claude will offer a health check: security updates, anything a platform is about to stop supporting, and errors on the live app.

**Every later session:** open the folder, run `claude`, type `build`. Claude reads `docs/PROGRESS.md`, tells you where things are, and continues. Want something specific? Type `build` and then say what you want ("add a way to export to Excel"). It slots it in as a feature, a small change or a bug. Got an idea for later? Say "add to the backlog: …", or write it in `docs/BACKLOG.md` yourself.
